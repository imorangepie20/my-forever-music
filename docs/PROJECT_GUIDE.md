...
- TIDAL 트랙은 `useTidalAnalyserAdapter` 훅이 `tidalStreamPlayback.getTidalAudioElement()` 로 받은 `<audio>` 요소에 `AudioContext` + `MediaElementAudioSourceNode` + `AnalyserNode`(fftSize 256) 체인을 부착해 실시간 frequency data 를 N 개 bar 로 downsample 함. `attachedElements` `WeakMap` 으로 동일 요소에 중복 부착하지 않음. CORS-tainted 등으로 `getByteFrequencyData()` 가 30 프레임 연속 0 만 반환하면 amber warning 을 surface 하고 Visualizer 는 procedural envelope 로 fallback 함. `<audio>.crossOrigin` 은 손대지 않음 — TIDAL CDN CORS 설정에 따라 재생 자체가 깨질 수 있어 보수적 결정. `createMediaElementSource` 는 audio output 을 영구히 graph 로 rerouting 하므로, AudioContext 가 `running` 이 아니면 호출하지 않고 user gesture(pointerdown/keydown) 후 resume 이 성공해야 attach 함 — gesture 전에는 audio 가 기존 HTML5 default pipeline 으로 흐름
