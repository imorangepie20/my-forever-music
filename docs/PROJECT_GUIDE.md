...
- `services/api` 의 `melon` 모듈은 Jsoup 으로 `https://www.melon.com/chart/index.htm` 을 파싱해 `melon_chart_track` 테이블에 최신 100곡을 저장한다. `POST /api/v1/admin/melon/scrape` 가 수동 트리거 (Phase 1, 자동 스케줄러는 Phase 2). `GET /api/v1/main-page/melon-hot-100?limit=10` 과 `/melon-hot-100/full` 두 엔드포인트가 데이터를 노출하며, `apps/web` 의 `MelonHot100Section` 이 메인 페이지 §6 에서 top 10 + "View all 100" 링크를, 신규 `/melon-hot-100` 라우트의 `MelonHot100Page` 가 전체 100곡 리스트를 표시한다. 클릭 시 외부 Melon URL 로 이동 (내부 재생은 Phase 2)
