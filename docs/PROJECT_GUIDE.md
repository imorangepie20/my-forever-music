...
- `apps/web`는 `/playback/visualizer` 페이지에 막대 64개 audio visualizer를 띄우고, PlaybackDock의 queue 칩 클릭 + 새 Maximize 버튼이 같은 라우트로 navigate함. `Visualizer` 컴포넌트는 `heightsAt(sample)` adapter를 prop으로 받으며, 미지정 시 mode별 procedural envelope(idle/spotify/tidal)로 fallback함. `usePlayback()`이 노출하는 `currentItem.sourcePlatform`으로 mode 자동 결정
- Spotify 트랙은 `useSpotifyAudioAnalysisAdapter` 훅이 `fetchPlaybackCredentials(userId, 'spotify')`로 access token을 받아 `GET https://api.spotify.com/v1/audio-analysis/{trackId}`를 호출하고, 응답을 캐시한 뒤 `positionMs`(ref로 동기화)를 기준으로 segment pitches × loudness × beat envelope를 합성한 `heightsAt`을 Visualizer에 주입함. 401/403/404 같은 거부는 silent fallback 대신 페이지에 amber warning + 원인 메시지로 surface함(`REAL_IMPLEMENTATION_POLICY` 준수)
- TIDAL 트랙은 `useTidalAnalyserAdapter` 훅이 `tidalStreamPlayback.getTidalAudioElement()` 로 받은 `<audio>` 요소의 `captureStream()` 결과를 `MediaStreamAudioSourceNode` 로 받아 `AnalyserNode`(fftSize 256) 에 흘리고 N 개 bar 로 downsample 함. 핵심 원칙: **audio 의 실제 재생 path 는 절대 가로채지 않는다** — captureStream 은 parallel observation 이라 analyser 가 어떤 상태로 가도 음향이 끊기지 않음. 이전에는 `createMediaElementSource` 로 audio output 을 graph 에 redirect 하다가 AudioContext suspended / 페이지 이동 / 다른 source 재생 같은 상황에서 PMS/EMS 재생까지 silent 되는 회귀가 있었음. `attachedElements` `WeakMap` 으로 동일 요소에 중복 부착하지 않고, `<audio>.crossOrigin` 은 손대지 않음 (TIDAL CDN CORS 설정에 따라 재생 자체가 깨질 수 있어 보수적 결정). AudioContext 가 `running` 이 아니면 attach 를 미루고 user gesture(pointerdown/keydown) 후 resume 이 성공해야 attach. zero-frame 감지는 `audio.paused === false` + `context.state === 'running'` 가드를 모두 통과한 경우만 카운트
- 위 Spotify/TIDAL adapter 두 종은 추가 회귀 다수 발생으로 모두 폐기 예정. 새 설계 계약은 [`architecture/PLAYBACK_VISUALIZER_DESIGN.md`](/Users/woosungjo/music-space/my-forever-music/docs/architecture/PLAYBACK_VISUALIZER_DESIGN.md) — 메인 재생 element에 Web Audio attach 전면 금지(`createMediaElementSource`, `captureStream` 둘 다), 1차는 metadata procedural envelope (BPM/energy/valence from ReccoBeats fill) 만, Spotify `audio-analysis` 사용 안 함(2024-11-27 신규 앱 제한). 실제 FFT가 정말 필요해지면 별도 `/dev/visualizer-harness` 격리 페이지에서 4개 환경 검증 후 도입
