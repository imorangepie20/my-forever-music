...
- `apps/web`는 `/playback/visualizer` 페이지에 막대 64개 audio visualizer를 띄우고, PlaybackDock의 queue 칩 클릭 + Maximize 버튼이 같은 라우트로 navigate함. `Visualizer` 컴포넌트는 `usePlayback()`의 `currentItem.sourcePlatform`으로 mode(idle/spotify/tidal)를 결정하고 내장 procedural envelope으로 막대를 그림. Web Audio API 호출은 0회 — Spotify SDK / TIDAL `<audio>` 어느 쪽도 가로채지 않음
- 위 결정의 기술 근거(외부 표준 인용, 금지 API 목록, 회귀 방어 검증)는 [`architecture/PLAYBACK_VISUALIZER_DESIGN.md`](/Users/woosungjo/music-space/my-forever-music/docs/architecture/PLAYBACK_VISUALIZER_DESIGN.md) 참고
- `services/api`는 `GET /api/v1/pms/tracks/{audioFeatureTrackId}/audio-features?user_id=...` 로 PMS user library 안의 ReccoBeats 보강 audio_feature(tempo/energy/valence/key 등)를 단일 트랙 단위로 조회한다. row가 없거나 `audio_features_filled=false` 인 경우 `unresolved` 응답을 반환해 visualizer가 mode preset envelope로 fallback 하게 함
- `apps/web`의 `/playback/visualizer`는 위 endpoint를 `useTrackAudioFeatures` 훅으로 호출하고, `proceduralEnvelope.makeHeightsAt(features)` 결과를 Visualizer에 주입한다. 보강된 경우 라벨이 "신호: BPM/energy envelope (tempo=…, energy=…)", 미보강이면 "신호: mode preset (audio_feature 미보강)" 으로 표시되어 운영자가 신호원을 즉시 확인 가능
