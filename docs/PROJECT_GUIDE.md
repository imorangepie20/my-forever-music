...
- 위 결정의 기술 근거(외부 표준 인용, 금지 API 목록, 회귀 방어 검증)는 [`architecture/PLAYBACK_VISUALIZER_DESIGN.md`](/Users/woosungjo/music-space/my-forever-music/docs/architecture/PLAYBACK_VISUALIZER_DESIGN.md) 참고. 1차는 mode preset, 후속 PR에서 ReccoBeats fill의 BPM/energy/valence를 사용한 envelope 추가 예정
