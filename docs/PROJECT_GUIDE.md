...
- `apps/web`는 `/playback/visualizer` 페이지에 막대 64개 audio visualizer를 띄우고, PlaybackDock의 queue 칩 클릭 + 새 Maximize 버튼이 같은 라우트로 navigate함. `Visualizer` 컴포넌트는 `heightsAt(sample)` adapter를 prop으로 받으며, 미지정 시 mode별 procedural envelope(idle/spotify/tidal)로 fallback함. `usePlayback()`이 노출하는 `currentItem.sourcePlatform`으로 mode 자동 결정. 후속 commit에서 Spotify `audio-analysis` 기반 adapter와 TIDAL `<audio>` element 위 `AnalyserNode` FFT adapter가 `heightsAt`에 주입될 예정
