...
- `services/api`는 `GET /api/v1/main-page/magazine-articles?limit=N`(기본 2) 을 제공함. `MagazineArticleService` 가 `ems_acquisition_signal` 테이블에서 article_url 기준 distinct 최신 레코드를 잘라 sourceName/articleUrl/articleTitle/rationale/capturedAt 으로 노출하며, 각 row 마다 `MagazineArticleEnricher` 가 (a) Google unofficial translate (`translate.googleapis.com/translate_a/single`) 로 title/rationale 한국어 번역 (b) article URL 에서 `og:image`/`twitter:image` meta 스크래핑으로 썸네일을 채워 응답에 함께 포함함. 두 결과 모두 module-level LRU 캐시(이미지 256, 번역 512). 적재 주기는 기존 `EmsAcquisitionScheduler` 의 24h fixed-delay 를 그대로 사용 (env `EMS_ACQUISITION_ENABLED=true`). `apps/web` 의 `MagazineSection` 이 메인 페이지에서 2열 큰 카드로 표시하고(헤드라인은 한국어 우선, 원문은 보조 노출) 카드 클릭 시 원본 기사 새 창으로 이동
