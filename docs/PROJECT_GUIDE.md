...
- TIDAL 트랙은 `useTidalAnalyserAdapter` 훅이 `tidalStreamPlayback.getTidalAudioElement()` 로 받은 `<audio>` 요소에 `AudioContext` + `MediaElementAudioSourceNode` + `AnalyserNode`(fftSize 256) 체인을 부착해 실시간 frequency data 를 N 개 bar 로 downsample 함. 그래프 토폴로지는 병렬(`source → destination` audio path + `source → analyser` 관측 path) — analyser 가 어떤 상태로 가도 음향이 끊기지 않음. `attachedElements` `WeakMap` 으로 동일 요소에 중복 부착하지 않고, `<audio>.crossOrigin` 은 손대지 않음 (TIDAL CDN CORS 설정에 따라 재생 자체가 깨질 수 있어 보수적 결정). `createMediaElementSource` 는 audio output 을 영구히 graph 로 rerouting 하므로 AudioContext 가 `running` 이 아니면 호출하지 않고 user gesture 후 resume 이 성공해야 attach 함. AudioContext 가 mid-playback 에 suspended 로 빠지면 `statechange` 리스너가 즉시 resume 을 시도하고 실패하면 다음 gesture 에서 자동 재개. zero-frame 감지는 `audio.paused === false` + `context.state === 'running'` 가드를 모두 통과한 경우만 카운트 (이전엔 버퍼링 중에도 false-positive 가 발생함)
