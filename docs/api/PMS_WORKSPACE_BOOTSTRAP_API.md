...
이 문서는 `apps/web`의 PMS 화면이 초기 seed 데이터와 플레이리스트 후보를 불러올 때 사용하는 공개 API 계약입니다.
...
- 현재는 `3단계 소스` 구조다
- `user_id`가 있고 해당 사용자의 import 결과가 있으면, 그 데이터가 가장 먼저 사용된다
- 그다음 `database` 같은 DB 활성 프로필에서는 `PostgreSQL + Flyway`로 준비된 `pms_*` 테이블에서 실제 데이터를 읽는다
- 마지막 fallback으로 `local` 프로필의 Spring Boot 내부 bootstrap 데이터를 사용한다
...
- import 기반 bootstrap에서는 가장 최근에 PMS로 들여온 playlist를 기본 workspace playlist로 사용한다
...
- sandbox import로 들어온 track도 이 응답에서 같은 방식으로 내려간다

## 다음 연결 지점

1. playlist owner를 실제 사용자 엔터티와 더 명확히 연결
2. import bootstrap과 DB bootstrap을 공통 영속 모델로 합치기
3. seed suggestion을 저장된 청취 이력과 연결
4. PMS에서 선택한 값이 이후 EMS / GMS 호출과 사용자 저장 상태로 이어지게 확장
5. 현재 demo seed migration과 sandbox import를 실제 ingest/관리 흐름으로 교체
