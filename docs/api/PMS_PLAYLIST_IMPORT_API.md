...
- `tidal`은 연결된 실제 credential이 있으면 import 후보에 노출한다. 단, 운영 안정화 단계는 아직 `testing-provider`다
- `youtube-music`, `apple-music`은 실제 provider 구현과 검증이 끝날 때까지 import 후보에 노출하지 않는다
...
- 현재 `spotify` provider는 audio-features 조회가 가능하면 그 값을 저장하고, 조회가 실패하면 import를 계속 진행하면서 `audio_feature_source=unavailable`, `audio_features_filled=false` 스냅샷을 저장한다
- 현재 `tidal` provider는 TIDAL `ISRC`를 기준으로 ReccoBeats 후보를 고르고, 일치하는 항목이 없으면 import를 계속 진행하면서 `audio_feature_source=unavailable`, `audio_features_filled=false` 스냅샷을 저장한다
- TIDAL playlist/profile/items provider 호출 실패는 빈 playlist로 숨기지 않고 `502 Bad Gateway`, `code=platform_provider_operation_failed`로 원인을 노출한다
...
1. 실제 TIDAL 계정으로 end-to-end import를 반복 검증하고 응답 shape 차이를 수집
