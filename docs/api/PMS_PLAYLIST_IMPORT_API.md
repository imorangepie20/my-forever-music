...
- import 저장소도 현재는 `in-memory`이며, 서버 재시작 시 초기화된다
...
2. Apple Music / TIDAL provider도 실제 플랫폼 API로 확장
3. import 결과를 JPA / PostgreSQL 영속 저장으로 확장
4. fallback 오디오 특성 생성 로직을 더 정교한 보강 파이프라인으로 교체
5. import 완료 후 사용자 행동 이벤트와 재학습 큐까지 연결
