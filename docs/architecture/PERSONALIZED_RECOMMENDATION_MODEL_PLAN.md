...
- Phase 2 metadata normalization 1차 진입: `MusicBrainzClient` 추가 (recording 검색 read-only, 익명 rate limit 준수용 User-Agent 헤더). 관리자 전용 `GET /api/v1/recommendations/admin/metadata/musicbrainz/recordings?user_id&title&artist&limit` endpoint 가 title + artist 로 MBID/ISRC/release 후보를 반환. 저장/confidence rule/Wikidata·Discogs 통합은 다음 단계.
