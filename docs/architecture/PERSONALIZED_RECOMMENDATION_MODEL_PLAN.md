...
- GMS playlist 흐름 1차 진입: `GET /api/v1/gms/playlists/preview?user_id&limit` endpoint 추가. 사용자가 cold-start 면 409 반환, 그렇지 않으면 EMS 본 테이블에 적재된 공개 playlist 중 사용자 PMS user_library 의 artist 집합과 curator/title 일치 + audio feature coverage + 트랙 수 기반의 단순 affinity score 로 정렬해 상위 N(기본 12, max 50) 후보를 반환. 응답은 model_stage, affinity_score, confidence_score, track_count, audio_feature_filled_count 등 포함. PMS save 흐름 + UI + 정교한 6축 기반 ranking 은 다음 단계.
