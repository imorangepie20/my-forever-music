...
- Phase 2 3차: 관리자 전용 `/recommendations/metadata-admin` 화면 추가. title/artist/limit/persist 입력으로 MusicBrainz lookup 결과 표시 + `track_identity_candidate` 목록을 status 필터(pending/accepted/rejected/all)로 조회, 각 row 에서 accept/reject를 `ConfirmDialog` 로 처리. Sidebar admin 메뉴에 "Metadata Normalize" 등록. confidence rule / Wikidata·Discogs 통합 / auto-resolve worker 는 다음 단계.
