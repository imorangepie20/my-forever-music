CREATE TABLE pms_user_playlist (
    user_playlist_id BIGSERIAL PRIMARY KEY,
    ...
    CONSTRAINT uk_pms_user_playlist_user_playlist UNIQUE (user_id, playlist_id)
);

CREATE INDEX idx_pms_user_playlist_user_synced_at
    ON pms_user_playlist (user_id, last_synced_at DESC, playlist_id ASC);

CREATE TABLE pms_user_track (
...
CREATE TABLE pms_user_playlist_track (
    user_playlist_track_id BIGSERIAL PRIMARY KEY,
    user_playlist_id BIGINT NOT NULL REFERENCES pms_user_playlist (user_playlist_id) ON DELETE CASCADE,
    track_id VARCHAR(160) NOT NULL REFERENCES pms_user_track (track_id),
    sort_order INTEGER NOT NULL,
    is_seed BOOLEAN NOT NULL DEFAULT FALSE,
    CONSTRAINT uk_pms_user_playlist_track UNIQUE (user_playlist_id, track_id),
    CONSTRAINT uk_pms_user_playlist_track_sort UNIQUE (user_playlist_id, sort_order)
);

CREATE INDEX idx_pms_user_playlist_track_playlist_sort
    ON pms_user_playlist_track (user_playlist_id, sort_order, user_playlist_track_id);
