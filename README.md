...
- `apps/web` 메인 페이지 `MagazineSection` 이 `GET /api/v1/main-page/magazine-articles?limit=2` 응답으로 RSS 매거진(Pitchfork/Stereogum/NME/BrooklynVegan/FACT/FADER/Billboard/Rolling Stone 등) 최신 기사 2건을 큰 카드(2열)로 노출. 데이터는 기존 `EmsAcquisitionScheduler`(env `EMS_ACQUISITION_ENABLED=true` 일 때 24h 주기 RSS 적재)가 `ems_acquisition_signal` 테이블에 적재하는 article_url/title/rationale 을 재활용. 카드 헤드라인은 한국어로 번역(Google unofficial translate `translate.googleapis.com` 호출, 결과 in-memory LRU 캐시)하고 원문은 작은 글씨로 보조 노출. 썸네일은 article URL 의 `og:image`/`twitter:image` meta 태그를 lazy 스크래핑해 LRU 캐시. 별도 신규 스케줄러 없이 기존 acquisition 주기로 갱신됨
