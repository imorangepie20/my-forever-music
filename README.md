...
- `apps/web`는 `/playback/visualizer` 페이지로 막대형 audio visualizer를 노출함. PlaybackDock의 queue 칩과 새 Maximize 버튼이 같은 페이지로 이동하고, `usePlayback().currentItem.sourcePlatform`을 기준으로 spotify/tidal/idle mode를 자동 결정함. 현재는 procedural envelope로 동작하며, Spotify audio-analysis 어댑터와 TIDAL AnalyserNode 어댑터는 후속 commit에서 `heightsAt` prop으로 주입됨
