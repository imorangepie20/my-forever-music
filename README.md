...
- TIDAL 트랙 재생 중에는 `useTidalAnalyserAdapter` 훅이 `tidalStreamPlayback.getTidalAudioElement()` 로 받은 `<audio>` 요소에 Web Audio `AnalyserNode`(FFT 256)를 부착해 실시간 frequency data 를 bar heights 로 매핑함. CORS-tainted 같은 이유로 AnalyserNode 가 0 데이터만 받으면 (재생은 계속됨) 30 프레임 후 amber warning 을 노출하고 procedural envelope 로 fallback 함
